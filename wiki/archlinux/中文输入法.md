...
常用的输入法框架有 Fcitx5 和 IBus。GNOME 桌面和 IBus 的兼容会更好，配置过程也更简单，所以推荐使用 IBUS。除 GNOME 外的其他桌面和合成器都推荐使用 Fcitx5。
...
1. 安装 IBus-rime
    ...
    ```bash
    sudo pacman -S ibus ibus-rime rime-ice-git
    ```
...
2. 安装中文输入方案
    ...
    - RIME 中州韵引擎+雾凇拼音
      ...
      6. 可选：默认使用英文标点
            ...
            雾凇拼音默认输出中文标点（，。？！）。如果你日常聊天、写代码更习惯英文标点，可以让它默认就处于英文标点状态，不用每次手动切换：
            ...
            ```yaml
            patch:
              # switches 是方案里的开关列表，@1 即第二项「中英标点」开关
              # reset: 1 表示每次启动都重置到第二个状态，也就是英文标点
              "switches/@1/reset": 1
            ```

            >如果前面配置语法模型时已经创建过这个文件，把 `"switches/@1/reset": 1` 加进已有的 `patch:` 里即可，不要出现两个 `patch:`。

            重启输入法生效。之后想临时切回中文标点，按 `Ctrl+Shift+3` 即可。
...
接下来的内容涉及有关如何让输入法正常工作，可能会稍微有一些复杂，建议让 AI 辅助进行阅读，如果不想看的话可以按照下面这个代码块的命令进行设置，出问题再说：
...
6. 了解如何让输入法正常工作
   ...
   >[fcitx5_wiki:Using_Fcitx_5_on_Wayland](https://fcitx-im.org/wiki/Using_Fcitx_5_on_Wayland#GTK_IM_MODULE)

   让输入法正常工作主要有两种方式，一是用输入法模块走D-Bus跟输入法框架通信的传统方式，其二是用 Wayland 协议以 Wayland 合成器为中间人跟输入法框架通信的现代方式。
   
   对于前者，首先软件要适配输入法模块，我们再设置对应的环境变量告诉软件使用哪个模块，输入法就可用了。举个例子，typora 这个 Markdown 编辑器支持 GTK 的输入法模块，那么我们就可以设置 GTK 的输入法模块环境变量通过该模块使用输入法。
   
   对于后者，软件需要适配 Wayland 的 `text-input` 协议，输入法框架要支持`input-method`协议，合成器同时支持这两类协议，无须配置环境变量输入法就能正常工作。`text-input`协议和`input-method`协议的版本必须匹配，但通常无须在意这个事情。举个例子，GNOME 的文本编辑器支持 `text-input-v3`，fcitx5 支持 `input-method-v2`，Niri 同时支持 `text-input-v3` 和 `input-method-v2`，输入法就可用了。
   ...
   - `QT_IM_MODULES` 适用于 Qt6.7 及以后的版本的 Qt 应用。

   - `XMODIFIERS` XIM 协议。在 Wayland 环境下仅适用于 XWayland 兼容层运行的 Xlib/GTK2/Qt4 应用。

   - `SDL_IM_MODULE` 适用于 SDL 应用。这一项比较特殊，SDL 没有模块文件，XWayland 运行的 SDL 应用会走 XIM，Wayland 运行的 SDL 应用只要合成器提供了 `text-input-v3` 就直接走 Wayland 协议，这个环境变量只在合成器不提供 v3 协议时生效，让 SDL 改用 D-Bus 和输入法通信。
...
7. 了解配置文件位置
    ...
    - 用户空间环境变量
      ...
      `~/.config/environment.d/`目录下，需新建`*.conf`文件。`*.conf`中的星号叫`通配符`，代表任意字符。

    - 调整单个应用的环境变量或参数
    
      需要复制`/usr/share/applications`中的`.desktop`到用户个人的`~/.local/share/applications`中后修改`Exec=`行，这是该`.desktop`文件启动时使用的命令。手动处理比较繁琐，推荐从 AUR 安装`pins-git`进行编辑。

      环境变量用`env`命令在命令前传入：`Exec=env GTK_IM_MODULE=fcitx linuxqq`；命令行参数本质是命令的选项，所以接在命令的后面：`Exec=linuxqq --ozone-platform=wayland %U`。

    - 窗口管理器

      Niri、Hyprland这样的窗口管理器的配置文件中通常拥有配置环境变量的地方的。

8. 正确设置环境变量

    参考 Fcitx5 的 Wiki，以下是我推荐的配置
    ...
    写入以下内容：
    ```
    # 支持 XIM 协议的 XWayland 应用走 XIM
    XMODIFIERS=@im=fcitx
    
    # QT6.7 及以后的应用能走Wayland走Wayland，走不了 Wayland 走 Qt 的输入法模块
    QT_IM_MODULES="wayland;fcitx"
    
    # 其他 Qt 应用走 Qt 的输入法模块
    QT_IM_MODULE=fcitx

    # 在合成器不提供 v3 时生效，让 SDL 改用 D-Bus 和输入法通信。
    SDL_IM_MODULE=fcitx
    ```

    然后是 Electron 和 GTK 应用的设置
    
    ```
    # 编辑 gtk2.0 的配置文件，让 XWayland 运行的 GTK2.0 应用走 GTK 的输入法模块
    vim ~/.gtkrc-2.0
    ```
    写入：
    ```
    gtk-im-module="fcitx"
    ```
    然后是 GTK3.0 和 4.0 的配置文件，以 XWayland 运行的时候走 GTK 的输入法模块
    ```
    ...
    之所以在配置文件中写而不是在设置环境变量的地方写，是因为环境变量优先级最高，如果以 Wayland 运行的 GTK 软件被设置了 GTK 的输入法模块环境变量，可能会出现闪烁、错位、无法输入等异常。而写在 GTK 配置文件中的内容优先级是最低的，当软件以 Wayland 运行时这些设置会被代码中硬编码的回退值覆盖，所以这些设置对以 Wayland 运行的 GTK 软件不生效，由此避免了异常。
