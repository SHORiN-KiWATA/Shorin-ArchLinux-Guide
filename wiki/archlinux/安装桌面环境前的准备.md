...
2. 设置密码
   ...
   - 可选：取消多次输错密码后的锁定时间
      ...
      ```
      把 `deny = 3` 改成 `deny = 10`（或你想要的次数）

      >`deny = 0` 表示完全不锁定（允许无限次试密码），等于关闭暴力破解防护，不建议。想让 sudo/登录更宽松，把次数调大即可。
...
- 如果是 `HOOKS(base udev ...)` 的话：
   ...
   2. 添加 `resume=` 内核参数（指向你的 swap 分区；swap 文件场景指向其所在分区），否则休眠无法工作。编辑 `/etc/default/grub`，在 `GRUB_CMDLINE_LINUX_DEFAULT` 里追加：

      ```
      resume=UUID=<swap分区的UUID>
      ```

      用 `lsblk -f` 查 UUID，然后重新生成 GRUB 配置：
