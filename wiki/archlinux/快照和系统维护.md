...
- [snapper](快照和系统维护.md#snapper)
- [回档方法](快照和系统维护.md#回档方法)
- [滚挂和良好的系统使用习惯](快照和系统维护.md#关于滚挂和良好的系统使用习惯)
- [downgrade 回退更新](快照和系统维护.md#拓展内容downgrade)
- [拓展：手动快照回档](快照和系统维护.md#拓展内容手动快照回档)

现在我们已经完成了安装桌面环境前的所有准备工作，接下来进行快照的配置，再学习一些简单的日用口诀。

使用 Linux 时我们常常会尝试新东西，加之 Arch 更新策略比较激进，如果不了解一下系统维护的话指不定哪天系统就挂了。而快照可以在你尝试新东西前保存系统状态，方便试完后回档，也可以定时创建快照以防某天系统突然挂掉。
...
3. 激活快照启动项服务
   ...
   >如果你是通过 `/efi/grub/grub.cfg` 存根读取 `/boot/grub/grub.cfg` 的方式配置的 GRUB 引导，需要进行额外操作，看：[附录：GRUB 在 Btrfs 文件系统的存根配置方法](附录.md#grub-在-btrfs-文件系统的存根配置方法)。
...
5. 设置合理的快照策略
   ...
   - 修改设置
      ...
      可以使用 `set-config <配置项>` 设置，但是一个个调太麻烦了，建议直接编辑文件。
...
现在你学会了快照的创建方法，后续阅读时自行判断要不要创建快照。
...
- btrfs-assistant 命令行（推荐）
   ...
   2. 用 snapper 序号找到对应的 **btrfs-assistant 序号**
      ...
      >注意一个细节，这条命令会把 home 和 root 的快照排在同一个列表里。图中 snapper 序号为 `11` 的 home 快照对应着 btrfs-assistant 序号 `6`。
...
- 从 GRUB 菜单的快照启动项进入系统
   ...
   通常在系统完全无法正常登入时使用，进入后用 btrfs-assistant 回档，GUI 或者命令行都可以，方法同上。
...
## 遇到异常
   ...
   - 解决方法二：让快照可读（不推荐）
      ...
      - 如果是 `udev`
         ...
         无法使用 overlayfs：`grub-btrfs-overlayfs` 钩子没有 systemd（`sd-` 前缀）版本，systemd 基础的 initramfs 不能用此方法。
...
- 良好的使用习惯
  ...
  3. 不要部分更新
      ...
      一定要一次性更新所有软件，否则容易出现依赖问题。假设你的系统依赖版本是 `15`，使用过程中你`-Sy`更新了软件数据库，新版本软件的系统依赖可能是 `16`，这时候安装就会报`无法处理依赖`的错误了。另一个实例是`-Sy`更新了软件数据库，单独更新了系统依赖版本到`16`，而系统软件没更新，仍然依赖`15`，那么软件就无法使用了，严重的情况下会导致整个系统无法使用。这就是为什么`-Sy`是一个危险的`pacman`选项组合，安全的使用方法是`-Syu`，更新软件数据库后更新所有软件。

      >不过实际使用时，我偶尔会刻意用`-Sy`去更新某个软件，只要不更新依赖其实问题不大，`-Sy`后记得找个时间`-Syu`就行。 

  4. 密钥单独更新

      密钥是唯一可以单独更新的东西。当待更新软件列表出现了 `keyring` 的时候，先 `-Sy` 单独更新，然后再 `-Syu` 更新整个系统。

  5. 做不了解的事情要小心

      系统损坏的原因往往是用户自己的不当操作，明白自己的行为会造成怎样的后果，**养成习惯，每次做自己不了解的事情之前都创建快照**。

## 拓展内容：downgrade


<details><summary>有时候更新完之后可能反而不好用，这时就要使用 downgrade 退回之前的版本。</summary>

>降级到太老的版本可能会出现依赖问题；千万不要降级关键依赖。
...
万事俱备，接下来选择自己喜欢的桌面环境吧。记住，你随时可以回档到这个时间点，所以感兴趣就试试吧。

下一节：[选择桌面环境或窗口管理器](安装桌面环境或窗口管理器.md)
