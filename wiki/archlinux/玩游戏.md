...
4. 安装 ARM 转译
    ...
    >该项目更新很不活跃（2026 年初之后基本无维护），遇到问题需要自行排错；官方文档见仓库 README。
...
2. 关闭 CPU 的 UMIP 功能
    ...
    如果有输出的话说明 CPU 启用了 UMIP ，需要编辑内核参数写上 `clearcpuid=514` 禁用此功能（注意这会关闭一项 CPU 安全加固，仅在虚拟机游戏场景确有需要时使用）。

    ```
    sudo vim /etc/default/grub
    ```
    在现有的 `GRUB_CMDLINE_LINUX_DEFAULT`（或 `GRUB_CMDLINE_LINUX`）值**末尾追加** ` clearcpuid=514`，不要整行覆盖掉已有的其他参数：

    ```
    GRUB_CMDLINE_LINUX_DEFAULT="已有的参数... clearcpuid=514"
    ```
    ```
