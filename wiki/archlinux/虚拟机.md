...
3. 清理残留文件
   ...
   ```bash
   rm -rfv ~/.winboat ~/.config/winboat
   ```

   >⚠️警告⚠️：网上常见的卸载方法会执行 `sudo rm -rfv /var/lib/docker /etc/docker ~/.docker /var/run/docker`，这会删除 Docker 的**全部**数据（所有容器、镜像、卷），如果你还用 Docker 跑别的东西，**千万不要执行**。winboat 自身的数据只需要删上面两个目录，用 winboat 内置的重置功能即可处理它的 Windows 虚拟机磁盘。
