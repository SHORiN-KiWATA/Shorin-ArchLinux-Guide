...
>⚠️警告⚠️：热切换显卡直通属于高风险操作。过程中宿主机可能直接死机，需要强制断电重启，**请提前保存所有工作**；部分笔记本（ACPI 实现较差的机型）在卸载驱动后显卡可能无法软恢复，只能关机再开机。如果中途某一步失败，可以直接按文末的「关闭显卡直通」一节回滚到混合模式。直通切换期间合盖休眠、安全睡眠等功能可能出现异常，建议暂时不要使用。
