...
- 修改或者移除 GRUB 主题
    ...
    GRUB 主题文件存放在 `/usr/share/grub/themes`。运行 `sudo change-grub-theme` 可以修改主题。
...
- 生产力与多媒体
    ...
    | `LazyVim`（非软件包，见下方说明）                | 优秀的 Neovim 预设配置            |
    | `opencode`                                       | 开源 AI 助手                      |
    | `mpv`                                            | 视频播放器                        |
    | `imv`                                            | 图片查看工具                      |
    | `obs-studio`                                     | 推流与录屏                        |
...
>关于 `LazyVim`：它不是软件包（官方仓库和 AUR 都没有），一键脚本是通过 `git clone https://github.com/LazyVim/starter ~/.config/nvim` 安装的。手动安装请参考 [LazyVim 官方文档](https://www.lazyvim.org/)。
